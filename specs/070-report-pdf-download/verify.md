...
## 보완 구현 전 검증 (2026-09-16)
코디네이터의 SDD-070 보완 구현 브리프에 따라 기존 승인 범위에서 진행한다.
- [x] 웹과 동일한 6지표 전후 비교, 임계값, 반올림, 저장 changes 우선순위를 검증한다.
- [x] 타임라인 비율 스케일·HRV 대체·스트레스 역방향 및 결측 보존을 검증한다.
- [x] 카드/정의/방향 해석/요약 chips/시간 단계가 실제 4페이지 PDF에 포함된다.
- [x] 실제 타임라인 SVG의 결측 구간은 연결하지 않는다.
- [x] 전체 BE pytest, FE build, PDF 4페이지 PNG 확인을 수행한다.
